...
# Host unit tests for the pure modules (schedule, display formats, datetime, beep envelopes).
...
- **`test/` holds Unity suites** for `schedule.cpp`, `display_format.cpp`, `datetime_validation.cpp`, and `beep_pattern.cpp` - the modules that are pure by design. They run via `pio test -e native` wherever a host compiler exists; `test/stubs/` supplies host stand-ins for `Arduino.h` and RTClib's `DateTime`.
...
- `ClockApplication` owns `RtcService` from `rtc_ds3231.h` and injects it into the controller, display manager, and config API. **All of its state is in members**; the only file-static state is the three `volatile` counters the SQW ISR writes, which are genuinely singleton (one SQW pin) and kept out of the object so the `IRAM_ATTR` handler does not chase a pointer.
  ...
  - `begin()`, `getStatus()`, `getNow()` (live I2C read), and `setNow()` (also resyncs the cache) provide device operations. **`getNow()` and `getNowCached()` both gate on `status_.present`**; an ungated `rtc_.now()` against a missing device returns BCD garbage, not a sentinel.
  - **`timeIsTrustworthy()` is not `isHealthy()`.** `isHealthy()` asks whether SQW pulses are arriving; after lost-power recovery the pulse train is perfectly healthy while the time itself is the firmware build date, which passes every range check and is still wrong by however long ago the image was built. `status_.timeTrusted` is set when a chip answers with its own time, cleared by `recoverIfPowerWasLost()` and `flagInvalidTimeIfNeeded()`, and set again by `setNow()`. **Anything that writes a timestamp to disk must consult `timeIsTrustworthy()`**, never `present` or `isHealthy()`.
...
1. **`display_format.h/cpp`** - the clock/counting format catalog and pure renderers (no I/O).
   ...
   - **A format's identity is its key, not its table position.** Each row has a stable `key` (e.g. `"yyyy-mmdd-hhmm"`) that is what `/config.json` stores and what the web dropdowns post; rows may be added, removed, or reordered freely. An index is an in-memory detail only, converted at the serialization boundary. This exists because the old index-valued scheme had already produced three disagreeing claims about the default clock format. `tools/check_formats.py` enforces key uniqueness and default resolvability on every build.
...
3. **`display_manager.h/cpp`** - application-owned `DisplayManager`; the single entry point for all display state.
   ...
   - **Render invalidation is an explicit `renderInvalidated_` flag, not a zeroed timestamp.** A sentinel timestamp breaks at long uptime and across `millis()` wrap, which is the same reason the throttle comparisons use the unsigned-wraparound idiom rather than `<`/`>` on raw deadlines.
...
- **`scheduled_mode.h/cpp`** owns one `ScheduledModeController`, including the shared previous-decision/time tracker, weekly sunset cache, view mapping, message/beep pairing, and approach-alert selection. It is ticked on accepted RTC samples, never on a logging interval.
...
- Friday displays clock from Saturday sunset to Friday midnight, countdown to Friday sunset, then countdown to Saturday sunset. Friday midnight is a silent boundary. Only Friday sunset has a message and optional beep; both sunsets have their respective generated approach alerts.
...
- Trading retains two configured same-day sessions plus an enabled count. `isValidTradingSchedule()` requires ordered, separated, non-overlapping enabled sessions. It counts down through weekday opens/closes, then the next weekday's first open; holidays and early closes are not modeled.
  - **A session cannot cross midnight**: `isValidTradingSchedule()` requires `startMinute < stopMinute` with both under 1440, so a 22:00-02:00 session is rejected rather than silently wrapping. Times are `HH:MM` local wall-clock strings at the JSON boundary and **minutes after midnight** (`uint16_t startMinute`/`stopMinute`) internally; Saturday and Sunday both target Monday's session 1.
- Every Trading open/close can announce its message and optional beep. Only the first open and last close have generated approach alerts. An event beep briefly overrides an approach pattern; previews have highest priority.
- Trading's optional over-24-hour format remains a render-time decision in `ViewState.longFormatIndex`.

### Sound
- **`beep_player.h/cpp`**: application-owned `BeepPlayer`, injected into the controller and API. It generates PWM tones without filesystem access or blocking delays.
- **`beep_pattern.h/cpp`**: pure elapsed-time envelope. Four equal phases double the starting beep rate three times. Each beep occupies the end of its slot and lasts at most 100 ms or half the slot, whichever is shorter. The final beep ends at the boundary; stalled loops skip expired pulses.
- `sound.boundaryAlert` retains `enabled`, `boundary1`, and `boundary2`. Each pattern stores `toneHz` (100-5000), `totalDurationSeconds` (4-1200), and `startingBeatsHz` (1-10). Defaults are 880 Hz/40 s/2 Hz and 1320 Hz/40 s/2 Hz. Boundary 1 is Friday sunset / first Trading open; Boundary 2 is Saturday sunset / last Trading close.
- Controllers update the scheduled target on every accepted RTC second, using the SQW-derived monotonic phase. Mode changes and time synchronization cancel/rebase the target. Arrival inside an approach window resumes its current position.
- Optional event beeps last 150 ms: startup, countdown completion, Friday sunset, and Trading open use 880 Hz; Trading close uses 1320 Hz. Startup sounds after initialization so WiFi setup cannot hold the beep on. Live boundary eligibility remains shared with the message; rebases never announce past events.
- Playback priority is preview, then event beep, then scheduled pattern. Schedule updates cannot interrupt a preview. When an override finishes, the current scheduled position resumes. `stop()` silences both and suppresses the current scheduled occurrence until the target changes.
- `sound.enabled` gates automatic beeps; event booleans default off. Volume maps 0-100% to PWM duty 0-50%. Preview bypasses the master switch and uses saved volume.
- `/sound` owns all beep settings and pattern previews. `/format` owns display settings only. `POST /api/sound/test` accepts a `boundaryAlert` object or `stop: true`; no catalog endpoint remains. See [SOUNDS.md](SOUNDS.md).
...
- **`countup.start` holds an absolute datetime; `kCountupStartNow` ("now") means "never been set", not "start from the present".** The sentinel exists because no absolute datetime is a correct factory default - a shipped date would make a fresh device show an ever-growing elapsed time, which is the same trap `countdown.end` fell into. `ConfigApi::resolveCountupStart()` substitutes the RTC's current time, after validation so a 400 stamps nothing and an explicit start time in the payload wins. Everything downstream then sees an absolute instant.
  - **It only stamps when `RtcService::timeIsTrustworthy()`**, and it reads the clock itself rather than taking a `DateTime`, so no caller can hand it a time the RTC does not vouch for. The earlier signature made that the caller's problem, and with no DS3231 on the bus `getNowCached()` returned BCD garbage decoded from an absent device's `0xFF` reads - year 2165 - which a save then wrote to `/config.json` permanently. An untrusted clock leaves the sentinel in place and logs why.
  - **Both writes go through `ConfigApi::persistClockConfig()`**, which resolves and then saves. That exists so a save path added later cannot skip the step; `POST /api/config` and `POST /api/mode` are the only two today.
...
- **The shipped `activeMode` is `clock`, and `countdown.end` is a placeholder only.** Countdown needs an absolute target, and any date compiled into firmware or shipped in `data/config.json` is in the past by the time a device is powered on - which put a brand-new clock straight into `messages.final` and read as broken hardware. Clock has no default that can go stale. `kDefaultCountdownDatetime` is now reachable only when someone selects Countdown without setting an end time, which `/format` asks for in the same visit.
- **`data/config.json` must not restate a value `defaults.cpp` already provides.** Loading it is a patch over the defaults, so an absent field falls through; `countdown.end` was spelled in both and the two copies had drifted twelve weeks apart before anyone noticed. Ship a field there only when it must differ from the compiled default.
...
- `ClockConfig.messages` stores `splash`, `final`, `fridaySunset`, `tradingOpen`, and `tradingClose`; they serialize under `display.messages` and are sanitized with `sanitizeDisplayMessage` (max 12 printable ASCII characters). Every buffer in the chain - `MessageConfig`, `OverlayState::message`, `DisplaySettings::finalMessage`, `BoundaryCue::message` - is `kDisplayMessageLength` (`config.h`). The buffer is deliberately much larger than the 12-character cap: the shipped messages use leading spaces to position text across the three panels. Trading boundary defaults are `"OPEN"` and `"CLOSE"`.
- `ClockConfig.sound` stores master enable, volume, five optional event-beep booleans (`startupBeep`, `finalBeep`, `fridaySunsetBeep`, `tradingOpenBeep`, `tradingCloseBeep`), and the two boundary patterns under top-level `sound`. Legacy song-name fields are ignored; existing master, volume, and boundary settings remain readable. Missing fields retain defaults.
...
- `ClockApplication` owns `ConfigManager` and `WifiConnectionManager` and injects them into the web APIs. Configuration saves serialize the complete cached `DeviceConfig` through a verified temporary file and a recoverable backup/rename sequence. Boot prefers a readable JSON-object primary, then `/config.bak`; unreadable files remain intact if neither loads. A surviving backup is retained until the replacement is installed. This is recovery across multiple operations, not one atomic transaction.
  - **Defaults are written to disk only when both files are absent.** If either exists but neither parses, the application runs on in-memory defaults and leaves both files alone, so a corrupt config is never overwritten with defaults before it can be inspected.
  - A valid backup is loaded **even if renaming it back over the primary fails**; restoration is deferred with a log line and the backup stays the recovery source through the next save. A readable primary removes the backup, since it means the last save completed. An abandoned `/config.tmp` is never treated as a committed save.
...
- **`config_validation.h/cpp`** - sanitizers and conversions, with no hardware dependency. The count-up origin resolver deliberately does **not** live here: it needs the RTC, and a module described as "sanitizers and conversions" reaching into a device driver is how that description stops being true. Canonical home of `modeName(mode)` / `modeFromName(name, out)` / `sanitizeMode`, `sanitizeFormatIndex`, `sanitizeOptionalFormatIndex` (also accepts `kSameFormat`/-1), `sanitizeBrightness`, `sanitizeBlinkMinutes`, `sanitizeUtcOffsetMinutes`, `sanitizePrintableText`, `sanitizeDisplayMessage`, `sanitizeWifiConfig`. Do not redeclare these elsewhere.
...
- **`web_server.h/cpp`**: application-owned `WebPortal`; `begin()`, `handleClients()` (called every loop), `getNetworkInfo()`, and `scheduleReboot()`.
  ...
  - REST API: `GET /api/status` (device name, configured mode, and live demo state for the home page), `POST /api/config`, `GET /api/config`, `GET /api/formats`, `POST /api/sound/test`, `POST /api/mode`, `POST /api/brightness`, `GET|POST /api/time`, `POST /api/sunset`, `GET /api/zipcode/lookup`, `POST /api/demo/test`, `POST /api/message/test`, `POST /api/field-mismatch`, `GET /api/wifi/status`, `GET /api/wifi/scan`, `POST /api/wifi/connect`.
...
- `/sound` previews a generated pattern with one Play/Stop button. `POST /api/sound/test` returns `durationMs`; the page resets the button with a timer instead of polling. Inputs retain independent settings for both boundaries.
