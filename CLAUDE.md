...
- `ClockApplication` owns `ConfigManager` and injects it into the web APIs. It provides `loadClockConfig()` / `saveClockConfig(cfg)` / `loadWifiConfig()` / `saveWifiConfig(cfg)` / `sanitizeClockConfig(cfg)`. Saves read the existing `/config.json`, patch their section, and atomically replace the file (tmp + rename), so the other section is preserved.
