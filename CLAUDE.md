...
### Storage / config
- `ClockConfig` (in `config.h`) holds: `activeMode`; `display` (`DisplayConfig`) with clock format, brightness, and 12-hour setting; `countdown` and `countup` with their format and anchor text; `friday` with its phase formats; messages; locations; and timezone fields.
...
- `ConfigManager` singleton in `config.h/cpp`: `loadClockConfig()` / `saveClockConfig(cfg)` / `loadWifiConfig()` / `saveWifiConfig(cfg)` / `sanitizeClockConfig(cfg)`. Saves read the existing `/config.json`, patch their section, and atomically replace the file (tmp + rename), so the other section is preserved.
